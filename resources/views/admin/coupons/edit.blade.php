...
<div class="max-w-3xl">
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">Edit Coupon: <span class="font-mono">{{ $coupon->code }}</span></h2>
        </div>
        <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="number" name="minimum_order_value" id="minimum_order_value" value="{{ old('minimum_order_value', $coupon->minimum_order_value) }}"
                        step="0.01" min="0"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-500">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $coupon->start_date?->format('Y-m-d')) }}"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-500">
                </div>

                <div>
                    <label for="expiry_date" class="block text-sm font-medium text-gray-700 mb-1">Expiry Date</label>
                    <input type="date" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $coupon->expiry_date?->format('Y-m-d')) }}"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Leave empty for unlimited. Used: {{ $coupon->times_used }}</p>
                </div>
            ...
            <div class="flex items-center gap-4 pt-4 border-t border-gray-200">
                <button type="submit" class="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 font-medium">Update Coupon</button>
