        ...
        <form action="{{ route('admin.colors.update', $color) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="color" name="hex_code" id="colorPicker" value="{{ old('hex_code', $color->hex_code ?? '#000000') }}" class="w-12 h-10 rounded border border-gray-300 cursor-pointer">
                    <input type="text" id="hexInput" value="{{ old('hex_code', $color->hex_code ?? '#000000') }}" placeholder="#000000" class="w-32 px-4 py-2 border border-gray-300 rounded-lg" readonly>
                </div>
            </div>
