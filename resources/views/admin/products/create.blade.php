            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="work_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                                ...
                                @foreach($works as $work)
                                    <option value="{{ $work->id }}" {{ old('work_id') == $work->id ? 'selected' : '' }}>{{ $work->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div>
                        ...
                        <textarea name="short_description" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('short_description') }}</textarea>
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="mt-4">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="with_blouse" id="with_blouse" value="1" {{ old('with_blouse', true) ? 'checked' : '' }} class="rounded border-gray-300 text-amber-600" onchange="toggleBlouseLength()">
                        <span class="text-sm text-gray-700">Includes Blouse Piece</span>
                    </label>
                </div>
                <div id="blouse_length_field" class="mt-4" style="{{ old('with_blouse', true) ? '' : 'display:none' }}">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Blouse Length</label>
                    <input type="text" name="blouse_length" value="{{ old('blouse_length') }}" placeholder="e.g., 0.8 meters" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="meta_keywords" value="{{ old('meta_keywords') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="number" name="price" value="{{ old('price') }}" required min="0" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Discount (%)</label>
                        <input type="number" name="discount" value="{{ old('discount', 0) }}" min="0" max="100" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-3">
                    ...
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_bestseller" value="1" class="rounded border-gray-300 text-amber-600">
                        <span class="text-sm text-gray-700">Bestseller</span>
                    </label>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_preorder" value="1" class="rounded border-gray-300 text-amber-600">
            ...
            <div class="flex gap-3">
                ...
                <button type="submit" class="flex-1 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700">Create</button>
            </div>
        </div>
    </div>
</form>
...
<script>
    ...
    function addFaq() {
        ...
        div.innerHTML = `
            ...
            <div class="space-y-3">
                <input type="text" name="faqs[${faqCount}][question]" placeholder="Question" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                <textarea name="faqs[${faqCount}][answer]" placeholder="Answer" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></textarea>
            </div>
        `;
    ...
    function addColorVariant() {
        ...
        const colorOptions = availableColors.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
        ...
        div.innerHTML = `
            ...
            <div class="grid grid-cols-3 gap-3 mb-3">
                ...
                <div>
                    ...
                    <input type="number" name="product_colors[${index}][stock]" value="0" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                </div>
                <div>
                    <label class="block text-xs text-gray-600 mb-1">Price Adjustment (₹)</label>
                    <input type="number" name="product_colors[${index}][price_adjustment]" value="0" step="0.01" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                </div>
            </div>
            <div>
